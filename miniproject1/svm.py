...
ran_samples = np.random.choice(N_train, 10000, replace=False)
ran_features = np.random.choice(d, 100, replace=False)
clf = svm.SVC()
clf.fit(X_train[ran_samples][:,ran_features], y_train[ran_samples])
score = clf.score(X_train[:3000][:,ran_features], y_train[:3000])
