...
def selectFeature(X_train, y_train, X_test1, X_test2, alpha1):
    ...
    '''
    cols = [6,7,39,41,48,57,64,371,374]
    x_train = X_train[:, cols]
    x_test1 = X_test1[:, cols]
    x_test2 = X_test2[:, cols]
    return (x_train, x_test1, x_test2)

def addFeature(X_train, X_test1, X_test2):
    length = len(X_train[0])
    for i1 in range (length):
        for i2 in range (i1, length, 1):
            feature1_train = (X_train[:,i1] * X_train[:,i2]).reshape(-1,1)
            X_train = np.hstack((X_train, feature1_train))
            feature1_test1 = (X_test1[:,i1] * X_test1[:,i2]).reshape(-1,1)
            X_test1 = np.hstack((X_test1, feature1_test1))
            feature1_test2 = (X_test2[:,i1] * X_test2[:,i2]).reshape(-1,1)
            X_test2 = np.hstack((X_test2, feature1_test2))
    return (X_train, X_test1, X_test2)
...
(X_train, X_test1, X_test2) = addFeature(X_train, X_test1, X_test2)
d = len(X_train[0])
...
for p2 in p2_lst:
    ...
    result_lst.append([score1[1], score2[1], n1, n2, p1, p2, batch])
# print all results
str1 = 'The regularization alpha in the Lasso regression is %.6f ; '
str2 = ' %d features have been selected.'
